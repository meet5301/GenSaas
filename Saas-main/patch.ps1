...
$before = $lines[0..3158] -join "`n"
$after  = $lines[3227..($lines.Length-1)] -join "`n"
                    ...
                    return (
                      <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem" }}>
                        ...
                        <div style={{ padding: "1.25rem", border: "1px solid var(--color-border)", borderRadius: "12px", backgroundColor: "#FAFAFA" }}>
                          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "0.85rem" }}>
                            ...
                            <div style={{ display: "flex", gap: "0.4rem" }}>
                              <button onClick={() => setBackupSelections(COLS.map(c => c.key))} className="btn btn-secondary" style={{ padding: "0.25rem 0.6rem", fontSize: "0.75rem" }}>All</button>
                        ...
                        <div style={{ padding: "1.25rem", border: "1px solid var(--color-border)", borderRadius: "12px", backgroundColor: "#FAFAFA" }}>
                          ...
                          <div style={{ fontSize: "0.75rem", color: "var(--color-text-muted)", marginBottom: "1rem" }}>Upload a JSON backup. Only collections present in the file will be overwritten.</div>
                          {restorePreview ? (
                            <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
                              <div style={{ padding: "0.85rem", backgroundColor: "#FFF8E1", border: "1px solid #FFE082", borderRadius: "8px", fontSize: "0.82rem" }}>
                                <div style={{ fontWeight: 700, marginBottom: "0.35rem" }}>⚠️ Review before restoring</div>
                                <div style={{ color: "var(--color-text-muted)", marginBottom: "0.5rem" }}>
                                  Exported: {restorePreview.exported_at ? new Date(restorePreview.exported_at).toLocaleString("en-IN") : "Unknown"} · Schema v{restorePreview.schema_version || "1"}
                                </div>
                                <div style={{ display: "flex", flexWrap: "wrap", gap: "0.3rem" }}>
                                  {COLS.filter(c => restorePreview[c.key]).map(c => (
                                    <span key={c.key} style={{ padding: "0.15rem 0.45rem", borderRadius: "10px", backgroundColor: "rgba(148,63,63,0.1)", color: "var(--color-accent-red)", fontWeight: 600, fontSize: "0.7rem" }}>
                                      {c.icon} {c.label} ({restorePreview[c.key]?.length ?? 0})
                                    </span>
                                  ))}
                                </div>
                              </div>
                              <div style={{ display: "flex", gap: "0.5rem" }}>
                                <button
                                  ...
                                  onClick={async () => {
                                    ...
                                    setBackupLoading(true);
                                    try {
                                      const res = await fetch(`${BACKEND_URL}/api/stores/${store.id}/restore`, {
                                        method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(restorePreview)
                                      });
                                      if (!res.ok) throw new Error();
                                  ...
                                  className="btn btn-primary"
                                  style={{ flex: 1, justifyContent: "center", backgroundColor: "#D32F2F", borderColor: "#D32F2F" }}
                                >
                            ...
                            <label
                              style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "0.5rem", padding: "1.5rem", border: "2px dashed var(--color-border)", borderRadius: "10px", cursor: "pointer", backgroundColor: "#FFFFFF" }}
                              onDragOver={e => e.preventDefault()}
                              onDrop={e => {
                                e.preventDefault();
                                const file = e.dataTransfer.files[0]; if (!file) return;
                                const reader = new FileReader();
                                reader.onload = ev => { try { setRestorePreview(JSON.parse(ev.target?.result as string)); } catch { alert("Invalid JSON file."); } };
                                reader.readAsText(file);
                              }}
                            >
                              <RefreshCw size={22} style={{ color: "var(--color-accent-red)" }} />
                              <span style={{ fontWeight: 600, fontSize: "0.88rem" }}>Click or drag & drop a backup JSON</span>
                        ...
                        <div style={{ padding: "1.25rem", border: "1px solid var(--color-border)", borderRadius: "12px", backgroundColor: "#FAFAFA" }}>
                          ...
                          <form onSubmit={async e => {
                            ...
                            try {
                              const res = await fetch(`${BACKEND_URL}/api/stores/${store.id}/warehouses`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name, location: loc }) });
                              if (res.ok) { const d = await res.json(); setWarehousesList(prev => [...prev, d]); (e.target as any).reset(); }
                            } catch { console.error("warehouse add failed"); }
...
$newContent = $before + "`n" + $newBlock + "`n" + $after
